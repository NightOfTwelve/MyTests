...
#include "BTDebugFlags.h"
#import <sys/timeb.h>

#define NSLog(...) NSLog(@"不是不让你用NSLog了嘛！")
 ...
 * 是否打印函数名&行号
 */
#define SHOULD_PRINT_FUNCTION_NAME_AND_LINE_NUMBER 1
...
if (SHOULD_PRINT_FUNCTION_NAME_AND_LINE_NUMBER)\
{\
...
printf("%s\n\n"\
,([[NSString stringWithFormat:__VA_ARGS__] cStringUsingEncoding:NSUTF8StringEncoding]));\
}while(0)
...
#ifdef DEBUG
#define CLog(condition,...) {	if ((BTDFLAG_SWITCH && condition)) { \
									ALog(__VA_ARGS__); \
								} \
							} ((void)0)
#else
...
#define CFLog(xx,...) CLog(xx,__VA_ARGS__)
#else
#define CFLog(xx,...) ((void)0)
#endif // #if BTLOGLEVEL_FATAL <= BTMAXLOGLEVEL

#if BTLOGLEVEL_SILENT <= BTMAXLOGLEVEL
#define CSLog(xx,...) CLog(xx,__VA_ARGS__)
#else
#define CSLog(xx,...) ((void)0)
