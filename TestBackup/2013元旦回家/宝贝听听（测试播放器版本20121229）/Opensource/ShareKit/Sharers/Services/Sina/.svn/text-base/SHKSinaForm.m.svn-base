...
- (void)segmentChanged:(id)sender {
    if ([sender selectedSegmentIndex] == 0) {
        ...
        }else if (textView.text.length == 0)
        {
            ...
            [[SHK currentHelper] hideCurrentViewControllerAnimated:YES];
            [(SHKSina *)delegate sendForm:self];
            if (ifFollowUs) {
                [(SHKSina *)delegate followMeOauth2];   //一键关注
            }
        }
    }
}
...
- (void)viewDidLoad {
	...
	//2012.11.29 nate add 内存泄漏
    UITextView *newUITextView = [[UITextView alloc] initWithFrame:self.view.bounds];
    self.textView = newUITextView;
    [newUITextView release];
    //2012.11.29 nate end
