...
@property (assign, nonatomic) id<ASIProgressDelegate> downloadProgressDelegate;
...
#pragma mark - Tea加密，Zero扩展
@property (copy) NSString *teaKey;

//当值为YES时，表明：告诉服务器，我可以处理Tea加密的返回。
//默认为YES
@property (assign) BOOL allowTeaEncryptedResponse;

//当值为YES时，表明：请求的BODY内容是经过Tea加密的
//默认为NO
@property (assign) BOOL shouldTeaEncryptRequestBody;

//加密后存放的路径（暂时用不上，没有处理发送文件的情况）
@property (retain, nonatomic) NSString *teaEncryptedPostBodyFilePath;

//Tea加密后的数据
@property (retain, nonatomic) NSData *teaEncryptedPostBody;

// Returns true if the response was tea-encrypted
