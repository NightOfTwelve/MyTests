...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setAllowTeaEncryptedResponse:YES];
...
//	NSMutableString *theValue = [NSMutableString stringWithString:[requestHeaders valueForKey:header]];
//	if (theValue != nil && theValue.length > 0) {
//		[theValue appendString:[NSString stringWithFormat:@";%@",value]];
//	}
...
#warning 从文件到文件，没有处理，目前应该用不上吧，等让Gary哥看看对不对
		//TODO: 从文件到文件，暂时没有处理 by Zero
//		if ([self shouldTeaEncryptRequestBody]) {
//			if (![self teaEncryptedPostBodyFilePath]) {
//				[self setTeaEncryptedPostBodyFilePath:[NSTemporaryDirectory() stringByAppendingPathComponent:[[NSProcessInfo processInfo] globallyUniqueString]]];
//				
//				NSError *err = nil;
//				NSString *sourceFilePath = nil;
//				if ([self shouldCompressRequestBody]) {
//					sourceFilePath = [self compressedPostBodyFilePath];
//				} else {
//					sourceFilePath = [self postBodyFilePath];
//				}
//				//TODO:
////				if (![ASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
////					[self failWithError:err];
...
////				}
//			}
//			path = [self teaEncryptedPostBodyFilePath];
//		}
		if (!([self shouldTeaEncryptRequestBody] || [self shouldCompressRequestBody])){
	...
	// Otherwise, we have an in-memory request body
		...
		{
			CVLog(BTDFLAG_NEW_TEA_CRYPT,@"post data before gzip, length:%u, data:%@",self.postBody.length,self.postBody);
		}
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
...
//		{
//			CVLog(BTDFLAG_NEW_TEA_CRYPT,@"post data before tea-crypt, length:%u, data:%@",self.postBody.length,self.postBody);
//		}
		if ([self shouldTeaEncryptRequestBody]) {
			{
				NSString *str = [[NSString alloc] initWithData:[self postBody] encoding:NSUTF8StringEncoding];
				CVLog(BTDFLAG_TEA_CRYPT,@"加密前:%@",str);
				[str release];
			}
			NSData *data = nil;
			if ([self shouldCompressRequestBody]) {
				data = self.compressedPostBody;
			} else {
				data = [self postBody];
			}
			
			{
				CVLog(BTDFLAG_NEW_TEA_CRYPT,@"post data before tea-crypt, length:%u, data:%@",data.length,data);
			}
			
			NSData *teaEncryptedBody = [KPCrypt encrypt:data withKey:_teaKey];
			[self setTeaEncryptedPostBody:teaEncryptedBody];
			
			{
				NSString *str = [[NSString alloc] initWithData:teaEncryptedBody encoding:NSUTF8StringEncoding];
				CVLog(BTDFLAG_TEA_CRYPT,@"加密后:%@",str);
				[str release];
			}
			
			{
				CVLog(BTDFLAG_NEW_TEA_CRYPT,@"post data after tea-crypt, length:%u, data:%@",self.teaEncryptedPostBody.length,self.teaEncryptedPostBody);
			}
			
			[self setPostLength:[[self teaEncryptedPostBody] length]];
		}
		if (!([self shouldCompressRequestBody] || [self shouldTeaEncryptRequestBody])) {
...
- (NSData *)responseData
{
	if ([self isResponseTeaEncrypted]) {
		[self setRawResponseData:[KPCrypt decrypt:[self rawResponseData] withKey:_teaKey]];
	}
	...
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		...
		[self setRawResponseData:[[data mutableCopy] autorelease]];
		CDLog(BTDFLAG_TEA_CRYPT,@"解密&&解压后的返回数据:%s",(const char *)[data bytes]);
	}
...
- (void)buildRequestHeaders
{
	...
	// Accept a tea-encrypted request body
	if ([self allowTeaEncryptedResponse]) {
		[self addRequestHeader:@"Accept-Encrypt" value:@"qencrypt"];
	}
	
	// Configure a tea-encrypted request body
	if ([self shouldTeaEncryptRequestBody]) {
		[self addRequestHeader:@"Content-Encrypt" value:@"qencrypt"];
	}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldTeaEncryptRequestBody] && [self teaEncryptedPostBody]) {
				[self setPostBodyReadStream:[ASIInputStream inputStreamWithData:[self teaEncryptedPostBody] request:self]];
			} else if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
...
#pragma mark - Tea加密，by Zero
- (BOOL)isResponseTeaEncrypted {
	NSString *encoding = [[self responseHeaders] objectForKey:@"Content-Encrypt"];
	return encoding && [encoding rangeOfString:@"qencrypt"].location != NSNotFound;
}
