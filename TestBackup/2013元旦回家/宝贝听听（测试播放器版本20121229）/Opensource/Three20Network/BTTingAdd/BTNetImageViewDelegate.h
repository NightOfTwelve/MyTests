...
@class BTNetImageView;

@protocol BTNetImageViewDelegate <NSObject>
@optional

/**
 * Called when the image begins loading asynchronously.
 */
- (void)netImageViewDidStartLoad:(BTNetImageView*)imageView;

/**
 * Called when the image finishes loading asynchronously.
 */
- (void)netImageView:(BTNetImageView*)imageView didLoadImage:(UIImage*)image;

/**
 * Called when the image failed to load asynchronously.
 * If error is nil then the request was cancelled.
 */
- (void)netImageView:(BTNetImageView*)imageView didFailLoadWithError:(NSError*)error;
 ...
 */
- (void)netImageView:(BTNetImageView*)imageView willSendARequest:(TTURLRequest*)requester;
