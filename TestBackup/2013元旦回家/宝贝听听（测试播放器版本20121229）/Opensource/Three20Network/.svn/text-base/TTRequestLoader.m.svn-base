...
- (BOOL)cancel:(TTURLRequest*)request {
  ...
  if (![_requests count]) {
    ...
    return NO;
      ...
      //DLog(@"_requests数量是%d",[_requests count]);
//      if (_connection) {
//          //DLog(@"warnning!!!!!!");
//          [_connection cancel];
//          TT_RELEASE_SAFELY(_connection);
//          [self load:[NSURL URLWithString:self.urlPath]];
//          
//      }
    return YES;
  }
...
- (void)dispatchLoaded:(NSDate*)timestamp loadFromCache:(BOOL)respondedFromCache{
    for (TTURLRequest* request in [[_requests copy] autorelease]) {
        request.timestamp = timestamp;
        request.isLoading = NO;
        request.respondedFromCache = respondedFromCache;
        for (id<TTURLRequestDelegate> delegate in request.delegates) {
            if ([delegate respondsToSelector:@selector(requestDidFinishLoad:)]) {
                [delegate requestDidFinishLoad:request];
            }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
  TTNetworkRequestStopped();
    ...
    int loadBytes = 0;
    for (TTURLRequest* request in [[_requests copy] autorelease]) {
        contentLength = request.totalContentLength;
        loadBytes =  request.totalBytesDownloaded;
        break;
    }
    ...
    //认为下载失败
    if(loadBytes != contentLength){
    //if(random){
        TT_RELEASE_SAFELY(_responseData);
        TT_RELEASE_SAFELY(_connection);
        //CDLog(Neoadd,@"不够0.9,下载失败");
        if(_retriesLeft){
            --_retriesLeft;
            [self load:[NSURL URLWithString:_urlPath]];
        }else{
            NSError *error = [NSError errorWithDomain:@"KPThree.downloadUnFinish" code:1 userInfo:nil];
            [_queue loader:self didFailLoadWithError:error];
        }
        return;
    }
  ...
  TTDCONDITIONLOG(TTDFLAG_ETAGS, @"Response status code: %d", _response.statusCode);
   // CDLog(Neoadd,@"图片成功了，，，%.2f",(float)loadBytes/(float)contentLength);
