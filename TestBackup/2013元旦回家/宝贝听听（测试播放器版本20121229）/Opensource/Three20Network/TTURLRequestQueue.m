...
static const NSInteger kMaxConcurrentLoads = 4;
...
- (id)init {
  ...
  if (self) {
    ...
    _maxContentLength = kDefaultMaxContentLength;
      _imageCompressionQuality = 1.0;//0.75;
...
- (void)loadNextInQueueDelayed {
    //DLog(@"%s,timer:%d",__FUNCTION__,(_loaderQueueTimer == nil));
...
- (void)loadNextInQueue {
    //DLog(@"%s _totalLoading = %d, _loaders:%d, _loaderQueue.count:%d",__FUNCTION__, _totalLoading, [_loaders count], _loaderQueue.count);
...
- (BOOL)sendRequest:(TTURLRequest*)request {
  ...
  TTRequestLoader* loader = nil;
    ...
    //DLog(@"add:request.cachekey= %@",request.cacheKey);
  ...
  if (![request.httpMethod isEqualToString:@"POST"]
    ...
    if (loader) {
//       DLog(@"我觉得这里有问题1,%d",[loader.requests count]);
//        TTURLRequest *olderRequest = [loader.requests objectAtIndex:0];
//        DLog(@"olderRequest = %@,newrequest = %@",olderRequest.cacheKey,request.cacheKey);
      [loader addRequest:request];
//        DLog(@"我觉得这里有问题2,%d",[loader.requests count]);
//            DLog(@"xxxxxxxtttttttt,%d,%d",[_loaders count],[_loaderQueue count]);
      return NO;
    }
  }
  ...
  [loader release];
    //DLog(@"____%d___%d__",[_loaders count],[_loaderQueue count]);
    
                                                                                                                                                                                                                                                              //DLog(@"++++++++++++++");
//    if([_loaders count]==[_loaderQueue count]){
//        DLog(@"!!!!!!!!!!!!!!!!!!!!!!!!!!!,%d",[_loaders count]);
//        DLog(@"################");
//        for(TTRequestLoader *loader in [_loaders allValues]){
//            DLog(@"1111111");
//            for(TTURLRequest *request in loader.requests){
//                DLog(@"%@,%d",request.cacheKey , loader.isLoading);
//            }
//            DLog(@"22222222");
//        }
//        DLog(@"++++++++++++++++");
...
- (void)cancelRequest:(TTURLRequest*)request {
  if (request) {
      //DLog(@" cacel 的request的cachekey = %@",request.cacheKey);
...
- (void)     loader: (TTRequestLoader*)loader
  ...
  [self loadNextInQueue];// fix bug in Three20Network: should invoke loadNextInQueueDelayed
}
